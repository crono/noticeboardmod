...
class action_plugin_noticeboard extends DokuWiki_Action_Plugin {
	...
	private function _buildHtmlDiff($id, $rev) {
	
		$df  		=	new Diff(explode("\n",htmlspecialchars(rawWiki($id,$rev))), /// tady porovnat $TEXT a id bez rev a mame vyhrano!
	...
	private function _notifyGroup() {
		...
		$bodyhtml = $this->_buildHtmlDiff($ID, $this->_getLastRev($ID	)) . $this->render($TEXT); //!!!!!! new or change???
